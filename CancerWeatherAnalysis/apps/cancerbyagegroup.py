...
dfAge = pd.read_csv('data/cancer_age_group.csv')
...
layout = html.Div([
       ...
       html.Div([
        ...
        dcc.Graph(id='Gender-graphic')
    ], className="row")

@app.callback(
    dash.dependencies.Output('Gender-graphic', 'figure'),
...
def bigfoot_class(xaxis_column_name,yaxis_column_name,cancertype):    
    ...
    dff1 = dff[dfAge['cancer_type'] == cancertype]
    return {
        "data": [
            {
                "type": "pie",
                "labels": dff1['gender'],
                "values": dff1[xaxis_column_name],
                "hole": 0.4,
                'name':'Test'
            }            
        ],
